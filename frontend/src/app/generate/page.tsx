...
export default function GenerateReportPage() {
  ...
  const validateForm = () => {
    ...
    if (!department) { setError("Please select a department"); return false; }
    if (!reportType) { setError("Please select a report type"); return false; }
    if (selectedMetrics.length === 0) { setError("Please select at least one metric"); return false; }
  ...
  try {
    ...
    // Navigate to the report page or show success message
    router.push("/report"); // or use router.push(`/report/${response.data.id}`) if you want details
  } catch (err: any) {
    console.error(err);
        ...
        <div className="p-8">
          <div className="mx-auto max-w-4xl space-y-6">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={reportType} onValueChange={setReportType}>
                    <SelectTrigger id="report-type"><SelectValue placeholder="Select report type" /></SelectTrigger>
                    <SelectContent>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {metrics.map(metric => (
                      <div key={metric.id} className="flex items-center space-x-2">
                        ...
                        />
                        <Label htmlFor={metric.id} className="text-sm font-normal cursor-pointer">{metric.label}</Label>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Schedule */}
                <div className="space-y-2">
                  <Label htmlFor="schedule-type" className="text-base font-semibold">Generation Schedule</Label>
                  <Select value={scheduleType} onValueChange={setScheduleType}>
                    <SelectTrigger id="schedule-type"><SelectValue placeholder="Select schedule type" /></SelectTrigger>
                    <SelectContent>
            ...
            <div className="flex justify-end gap-4">
              <Button variant="outline" onClick={() => router.push("/")} disabled={isGenerating}>Cancel</Button>
              <Button onClick={handleGenerateReport} disabled={isGenerating} className="min-w-[150px]">
                {isGenerating ? <><Clock className="mr-2 h-4 w-4 animate-spin" />Generating...</> :
                  scheduleType === "immediate" ? <><FileText className="mr-2 h-4 w-4" />Generate Report</> :
