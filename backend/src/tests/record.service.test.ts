...
describe("RecordService", () => {
  ...
  it("getRecordsByPatient returns records for a patient", async () => {
    ...
    expect(result).toEqual(mockData);
    expect(mockedRecord.find).toHaveBeenCalledWith({
  ...
  it("updateRecord updates record if exists", async () => {
    ...
    (mockedRecord.findByIdAndUpdate as jest.Mock).mockResolvedValue(
      updatedRecord
    );
    ...
    expect(mockedRecord.findByIdAndUpdate).toHaveBeenCalledWith(
      recordId,
      expect.anything(),
      { new: true }
    );
  ...
  it("deleteRecord deletes a record", async () => {
    ...
    (mockedRecord.findByIdAndDelete as jest.Mock).mockResolvedValue(
      deletedRecord
    );
